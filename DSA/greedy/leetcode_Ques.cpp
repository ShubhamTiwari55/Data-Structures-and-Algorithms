...
//leetcode ques no. 435 -> Non-Overlapping intervals
...
public:
    int eraseOverlapIntervals(vector<vector<int>>& intervals) {
        ...
        for(int i=1;i<intervals.size();i++){
            if(intervals[i][0]<lastEle) ans++;
            else lastEle = intervals[i][1];
        }       
        return ans;
    }
};

//leetcode ques no. 452 -> minimum number of arrows to burst ballons
...
public:
    int findMinArrowShots(vector<vector<int>>& points) {
        ...
        int lastELe = points[0][1];
        for(int i=1;i<points.size();i++){
            if(points[i][0]<=lastELe) ans++;
            else lastELe = points[i][1];
        }
